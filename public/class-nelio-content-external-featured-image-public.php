...
class Nelio_Content_External_Featured_Image_Public {
	...
	public function define_public_hooks() {
		...
		add_filter( 'init', array( $this, 'disable_original_nelioefi_hooks' ), 1 );

		// Featured Image Hooks.
		add_filter( 'init', array( $this, 'add_hooks_for_thumbnail_simulation' ) );
	...
	/**
	 * This function adds all the relevant filters for simulating that a post
	 * with an external featured image has a "WordPress thumbnail".
	 *
	 * @since  1.1.1
	 * @access public
	 */
	public function add_hooks_for_thumbnail_simulation() {
		...
		foreach ( get_post_types() as $post_type ) {
			...
			add_filter( "get_${post_type}_metadata", array( $this, 'maybe_simulate_post_thumbnail_attachment' ), 10, 3 );

		}//end foreach

	}//end add_hooks_for_thumbnail_simulation()

	/**
	 * This function removes all the filters that simulate that a post with an
	 * external featured image has a "WordPress thumbnail".
	 *
	 * @since  1.1.1
	 * @access public
	 */
	public function remove_hooks_for_thumbnail_simulation() {

		foreach ( get_post_types() as $post_type ) {
			remove_filter( "get_${post_type}_metadata", array( $this, 'maybe_simulate_post_thumbnail_attachment' ), 10, 3 );
		}//end foreach

	}//end remove_hooks_for_thumbnail_simulation()
	...
	public function maybe_simulate_post_thumbnail_attachment( $null, $object_id, $meta_key ) {
		...
		if ( ! is_string( $efi_url ) || ! strlen( $efi_url ) ) {
			...
			$this->remove_hooks_for_thumbnail_simulation();
			$featured_image = get_post_meta( $object_id, '_thumbnail_id', true );
			$this->add_hooks_for_thumbnail_simulation();
	 ...
	 * @access public
	 */
	public function maybe_add_efi_as_background( $html, $post_id, $size = false, $attr = array() ) {
	...
	private function get_placeholder_id() {
		...
		$source  = NELIO_CONTENT_PUBLIC_DIR . '/images/nc-efi-placeholder.png';
