...
class Nelio_Content_Reference {
	...
	/**
	 * Assuming someone suggested this reference, the name of the user that
	 * suggested it.
	 *
	...
	public function __construct( $reference = 0 ) {
		...
		if ( is_numeric( $reference ) ) {
			...
			if ( 0 === $this->ID ) {
				$this->post = json_decode( json_encode( array( // @codingStandardsIgnoreLine
					'post_title'  => '',
	...
	private function build() {
		...
		if ( $this->is_external() ) {
			...
			$this->author_twitter = get_post_meta( $this->ID, '_nc_author_twitter', true );
			...
			$this->author_twitter = ''; // TODO. Set author's Twitter.
	...
	/**
	 * Tries to fill the information of this reference automatically, using the
	 * Link Meta class.
	 *
	 * @since  1.0.0
	 * @access public
	...
	public function autoload() {
		...
		// Otherwise, let's try to automatically retrieve information about the given URL.
		$aux = new Nelio_Content_Link_Meta();
		$meta = $aux->get_url_meta_data( $this->url );

		// TODO. Set publication date.
		$this->set_title( $meta['title'] );
		$this->set_author_name( $meta['author'] );
		$this->set_author_email( $meta['email'] );
		$this->set_author_twitter( $meta['twitter'] );
	 ...
	 * some date.
	 *
	 * @param integer $advisor ID of the user that suggested this reference.
	...
	private function maybe_update_status() {
		...
		$values = array();
		array_push( $values, $this->get_title() );
		array_push( $values, $this->get_url() );
		array_push( $values, $this->get_author_name() );
		array_push( $values, $this->get_author_email() );
		array_push( $values, $this->get_author_twitter() );
		array_push( $values, $this->get_publication_date() );
	...
	public function json_encode() {
		...
		if ( $this->is_suggestion ) {
			...
			$result['suggestionAdvisorDisplayName'] = esc_html( $advisor_name );
